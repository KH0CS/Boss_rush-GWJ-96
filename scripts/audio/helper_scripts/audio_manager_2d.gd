...
## Audio manager for playing sound effects and music in 2D scenes.
## 
## SETUP: Make sure that Sound_Effect_Settings and Music_Settings scripts exist in project, then set up your 'Sound Effects' and 'Music Tracks' in the Inspector. 
...
##		play_audio_at_location(location, sound_effect_type) - Play a sound at a specific location (2D positional audio).
...
func play_audio_at_location(location: Vector2, type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			new_2D_audio.volume_db = sound_effect.get_randomized_volume_db()
			new_2D_audio.bus = _get_valid_bus_name(sound_effect.bus_name)
			new_2D_audio.pitch_scale = sound_effect.get_randomized_pitch_scale()
			new_2D_audio.max_distance = sound_effect.max_distance
			new_2D_audio.attenuation = sound_effect.attenuation
			new_2D_audio.panning_strength = sound_effect.panning_strength
			sound_effect.log_playing_2d(location, new_2D_audio.bus, new_2D_audio.volume_db, new_2D_audio.pitch_scale) # Debug log if debug_print is enabled for this sound effect
		...
		push_warning("AudioManager2D: Sound type not found: " + str(type))
...
func play_audio(type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			sound_effect.log_playing_non_positional(new_audio.bus, new_audio.volume_db, new_audio.pitch_scale) # Debug log if debug_print is enabled for this sound effect
...
var active_looping_sounds: Dictionary = {} ## Tracks active looping sounds by their loop type. Stores {loop_type: {player: AudioStreamPlayer/AudioStreamPlayer2D, parent_node: Node2D/null}}
...
func play_loop_at_position(parent_node: Node2D, loop_type: SoundEffect.SOUND_EFFECT_TYPE, has_start_clip: bool = false) -> void:
	# Validate parent node
	if parent_node == null or not is_instance_valid(parent_node):
		push_warning("AudioManager2D: Invalid parent_node for loop")
		return
	
	# If a loop is already active for this type, warn and don't start a new one
	if active_looping_sounds.has(loop_type):
		push_warning("AudioManager2D: Loop " + str(SoundEffect.SOUND_EFFECT_TYPE.keys()[loop_type]) + " already playing")
		return
	
	# Start the loop directly
	_start_loop_2d(loop_type, parent_node)
...
func stop_loop(loop_type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	...
	# Get the active loop data and player
	...
	# Stop and clean up the loop (works for both AudioStreamPlayer and AudioStreamPlayer2D)
					...
					if stream != null:
						...
						if clip_stream != null:
							...
							if clip_length > 0.05:
								...
								push_warning("AudioManager2D: Final clip for " + str(SoundEffect.SOUND_EFFECT_TYPE.keys()[loop_type]) + " is too short (< 50ms), skipping auto-stop")
				else:
					loop_data.erase("final_clip_timer")
			else:
				push_warning("AudioManager2D: Could not get playback for loop " + str(SoundEffect.SOUND_EFFECT_TYPE.keys()[loop_type]))
		else:
			push_warning("AudioManager2D: Loop player is invalid for " + str(SoundEffect.SOUND_EFFECT_TYPE.keys()[loop_type]))
	else:
		push_warning("AudioManager2D: Loop " + str(SoundEffect.SOUND_EFFECT_TYPE.keys()[loop_type]) + " is not currently playing")
...
var active_music_dict: Dictionary = {} ## Dictionary of active music instances keyed by TRACK_TYPE. Allows multiple simultaneous tracks (one per type) and fades between them.
...
func play_music(type: MusicTrack.TRACK_TYPE, fade_in_duration: float = 0.0) -> AudioStreamPlayer:
	...
	if music_tracks_dict.has(type):
		...
		push_warning("AudioManager2D: Music type not found: " + str(type))
		return null
...
#--------------- MANAGED PLAYERS ---------------#
# This section allows actual nodes of AudioStreamPlayer or AudioStreamPlayer2D existing elsewhere in the scene (for instance on an enemy scene) to be registered with the AudioManager for centralized management. 
...
# To use this, add the AudioManagerHelper script to any AudioStreamPlayer or AudioStreamPlayer2D node in your scenes and set a unique player name in inspector. The helper script will automatically register the player with the AudioManager2D. 
...
var managed_players: Dictionary[String, Node] = {} ## Dictionary of AudioStreamPlayer/AudioStreamPlayer2D nodes registered via AudioManagerHelper script.
...
func get_managed_player(player_name: String) -> Node:
	...
	if player and not is_instance_valid(player):
		push_warning("AudioManager 2D: Managed player '" + str(player_name) + "' is no longer valid (scene was unloaded)")
		return null
	return player
...
func _validate_all_audiotypes_content() -> void:
	...
	if not unassigned_soundeffect_types.is_empty():
		push_warning("AudioManager2D: SoundEffect(s) at index " + str(unassigned_soundeffect_types) + " still set to UNASSIGNED_SOUND")
	...
	if warn_on_missing_sound_assignments:
		...
		if not missing_sound_effect_values.is_empty():
			push_warning("AudioManager2D: Unused sound types: " + ", ".join(missing_sound_effect_values))
	...
	if not unassigned_tracks.is_empty():
		push_warning("AudioManager2D: MusicTrack(s) at index " + str(unassigned_tracks) + " still set to UNASSIGNED_MUSIC_TRACK")
	...
	if warn_on_missing_sound_assignments:
		...
		if not missing_music_track_values.is_empty():
			push_warning("AudioManager2D: Unused music types: " + ", ".join(missing_music_track_values))
...
func _get_valid_bus_name(requested: StringName) -> StringName:
	...
	if requested_text.is_empty():
		push_warning("AudioManager2D: Empty bus name, using Master")
		return &"Master"
	if AudioServer.get_bus_index(requested_text) == -1:
		push_warning("AudioManager2D: Bus '" + str(requested_text) + "' not found, using Master")
		return &"Master"
	return StringName(requested_text)


## Internal helper to start a positional 2D looping sound. Creates an AudioStreamPlayer2D as a child of [param parent_node] so it follows the parent's position automatically. Called by play_loop_at_position().
func _start_loop_2d(loop_type: SoundEffect.SOUND_EFFECT_TYPE, parent_node: Node2D) -> void:
	if sound_effect_dict.has(loop_type):
		var loop_sound: SoundEffect = sound_effect_dict[loop_type]
		if loop_sound.has_open_limit():
			loop_sound.change_audio_count(1)
			var loop_audio: AudioStreamPlayer2D = AudioStreamPlayer2D.new()
			
			# Parent to the node so it follows automatically
			parent_node.add_child(loop_audio)
			loop_audio.position = Vector2.ZERO # Relative to parent
			...
			loop_audio.max_distance = loop_sound.max_distance
			loop_audio.attenuation = loop_sound.attenuation
			loop_audio.panning_strength = loop_sound.panning_strength
			loop_sound.log_playing_2d(parent_node.global_position, loop_audio.bus, loop_audio.volume_db, loop_audio.pitch_scale) # Debug log if debug_print is enabled for this sound effect
			
			# Store the loop player reference and parent node
			active_looping_sounds[loop_type] = {
				"player": loop_audio,
				"parent_node": parent_node
			}
			
			# Clean up on finished (use stop_loop to stop properly, this is for safety)
...
func _start_loop(loop_type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(loop_type):
		var loop_sound: SoundEffect = sound_effect_dict[loop_type]
		if loop_sound.has_open_limit():
			loop_sound.change_audio_count(1)
			...
			loop_audio.bus = _get_valid_bus_name(loop_sound.bus_name)
			loop_audio.pitch_scale = loop_sound.get_randomized_pitch_scale()
			loop_sound.log_playing_non_positional(loop_audio.bus, loop_audio.volume_db, loop_audio.pitch_scale) # Debug log if debug_print is enabled for this sound effect
			
			# Store the loop player reference
			active_looping_sounds[loop_type] = {
				"player": loop_audio
			}
		...
		push_warning("AudioManager2D: Loop sound type not found: " + str(loop_type))
